...
__global__ void simple_reduction_kernel(float *input, int N, float *output) {
  ...
  // We can further use coarsening to increase the work done by each block
  // We can do this at the shared mem, where we partially reduce multiple sections into
  // one, instead of just two sections like in previous
  int segment = COARSE_FACTOR * blockDim.x * blockIdx.x;
...
int main() {
  ...
  int threads = 1024;
  int blocksPerGrid = (N/2/COARSE_FACTOR + threads - 1) / threads;
