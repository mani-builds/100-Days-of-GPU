...
__global__ void MatMulKernelRow(float *M_d, float *N_d, float *P_d, int M_Row_d,
                                int N_Col_d, int CommonDim_d) {
    int row = blockIdx.x * blockDim.x + threadIdx.x;
    if (row < M_Row_d) {
        for (int j = 0; j < N_Col_d; j++) {
            float element_value = 0.0;
            for (int i = 0; i < CommonDim_d; i++) {
                element_value +=
                    M_d[row * CommonDim_d + i] * N_d[N_Col_d * i + j];
            }
            P_d[row * N_Col_d + j] = element_value;
        }
    }
}
