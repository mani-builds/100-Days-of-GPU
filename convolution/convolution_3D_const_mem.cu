...
__constant__ float F[2*FILTER_RADIUS+1][2*FILTER_RADIUS+1][2*FILTER_RADIUS+1]; //= {0,1,0,1,0,1,0,1};

__global__
void convolution_3D_const_mem_kernel(float *N, float *P, int r, int width, int height, int depth){
...
//       }}}
  ...
  convolution_3D_const_mem_kernel<<<blocksPerGrid, threadsPerBlock>>>(N,P,r,width,height,depth);
